...
- Create a room or join **The living room**. Use **Video link** to paste a YouTube video or playlist URL, a Twitch VOD URL, or a public HTTP/HTTPS media file URL. Playlist submissions expand up to **200** playable entries per submission; queue size is capped at **500**. A URL with a `list` parameter adds the playlist. YouTube Mix/radio links retain their seed video so the generated playlist can be loaded.
