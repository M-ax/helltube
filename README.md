...
Desktop encoding prefers negotiated codecs that the browser's WebRTC Media Capabilities API reports as power efficient (usually hardware accelerated), using the captured resolution and frame rate. If capability detection is missing, rejects, or times out, it tries H.264 before VP8. If neither codec is reported as power efficient, supported codecs retain the relay's VP8-first preference. A local codec negotiation failure retries the other codec once with fresh browser and server transports, without reopening the screen picker. Connection/signaling failures keep the existing connection recovery behavior. The browser controls hardware/software encoder fallback; GPU encoding cannot be forced, and power efficiency is a hint rather than proof of GPU use. Capture remains capped at 1080p/30 fps, with the same bitrate ceilings.
...
The quality selector changes only your device, defaults to Original when ready, and preserves the room position when switching. The player falls back to a prepared Standard stream when its buffer drops below eight seconds while draining for two seconds, stays below six seconds for six seconds, or stalls with less than half a second buffered for three seconds. Initial loads and room seeks have a four-second grace period; pauses, autoplay blocks, disconnection, scrubbing and fully buffered endings do not trigger fallback. Standard stays selected for that video across seeks and retries; choosing Original manually clears the fallback, and the next video uses your preferred quality. On far seeks, chosen start times and backend restarts, Original seeks four seconds before the requested position, retaining the earlier keyframe. Its actual start is read from the first copied fragment before publication, including any earlier audio, so switching qualities preserves synchronization without copying the whole preceding video. Standard prepares the requested position concurrently. Both renditions and private staging count toward storage limits and are stopped and cleaned up together; a failed rendition does not stop the other.
