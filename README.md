...
For native monitor/window/region capture and per-application audio mixing on Windows or Linux, use **[Helltube Caster](apps/caster/README.md)**. It signs in with the frontend URL and your existing account, publishes through the same desktop relay, and includes bounded encoding presets and a live mixer. From this checkout, run `npm run caster:install`, `npm run caster:build-native`, then `npm run caster`; platform dependencies and packaging commands are in the caster guide.
