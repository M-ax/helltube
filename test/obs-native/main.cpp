...
int main()
{
    ...
    require(bitrate == 12000, "Incorrect video bitrate limit");
    obs_service_get_max_bitrate(service, nullptr, &bitrate);
    require(bitrate == 192, "Incorrect audio bitrate limit");
    auto *limitSettings = obs_data_create_from_json("{\"bitrate\":20000}");
    obs_service_apply_encoder_settings(service, limitSettings, nullptr);
    require(obs_data_get_int(limitSettings, "bitrate") == 12000, "Video bitrate must be capped at 12 Mbps");
    obs_data_set_int(limitSettings, "bitrate", 10000);
    obs_service_apply_encoder_settings(service, limitSettings, nullptr);
    require(obs_data_get_int(limitSettings, "bitrate") == 10000, "Video bitrate below 12 Mbps must be preserved");
    obs_data_release(limitSettings);
    auto *videoSettings = obs_data_create_from_json("{\"bitrate\":1500,\"preset\":\"veryfast\"}");
    auto *audioSettings = obs_data_create_from_json("{\"bitrate\":192}");
