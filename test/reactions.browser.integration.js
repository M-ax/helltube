...
test('pointing fingers track locally, stream at 60Hz, tap and slide together, and clean up', {timeout: 60000}, async t => {
    ...
    assert.equal(await a.evaluate(() => window.fingerAudio.crackles), crackles, 'Rubbing beside the recent trail produces no new static.');
    assert.equal(await b.evaluate(() => window.fingerAudio.crackles), crackles, 'Viewers share the same discharged trail.');
    ...
    await a.mouse.move(at(.55, .18).x, at(.55, .18).y);
    await a.mouse.down();
    for (let i = 0; i < 12; i++) {
        await a.mouse.move(at(.55 + i * .02, .18).x, at(.55 + i * .02, .18).y);
        await a.waitForTimeout(18);
    }
    await a.mouse.up();
    await until(async () => await a.evaluate(() => window.fingerAudio.crackles) > crackles);
    assert.equal(await b.evaluate(() => window.fingerAudio.crackles), mutedCrackles, 'Local sound mute also silences static.');
