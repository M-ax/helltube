...
// Each operation owns one dedicated browser process. No connection to everyday profiles.
export async function openBrowser(directory, browserName, visible, { executable, identity: selectedIdentity } = {}) {
  ...
  const child = spawn(executable || await browserExecutable(browserName), [
    ...
    ...(visible ? [] : ['--headless=new']), 'about:blank',
  ], { windowsHide: !visible, stdio: 'ignore' });
