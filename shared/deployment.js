...
export function securityHeaders(bareMetalOrigin = '') {
  ...
  return {
    ...
    'Content-Security-Policy': `default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' https://i.ytimg.com data:; media-src 'self' blob:${direct ? ` ${direct}` : ''}; worker-src 'self' blob:; connect-src 'self' ws: wss:${direct ? ` ${direct}` : ''}; object-src 'none'; base-uri 'none'; frame-ancestors 'none'`,
  };
}